#!/usr/bin/env bash
# Run the matched SAC RBF comparison serially across seeds on macOS or Ubuntu.
set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

CONFIG="${CONFIG:-configs/sac/rbf_comparison.yaml}"
SEED_LIST="${SEED_LIST:-42 123 456}"
RUN_TAG_PREFIX="${RUN_TAG_PREFIX:-sac-rbf}"
SLEEP_SECONDS="${SLEEP_SECONDS:-3}"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_DIR="outputs/sac_rbf_multiseed_${TIMESTAMP}"
...
{
    echo "SAC RBF multi-seed comparison started at $(date)"
...
for SEED in "${SEEDS[@]}"; do
    ...
    TMP_CONFIG="$(mktemp "${TMPDIR:-/tmp}/sac_rbf_seed${SEED}.XXXXXX")"
    ...
    echo "[$CURRENT/$TOTAL] seed=$SEED wall=${ELAPSED}s exit=$EXIT_CODE $(date)" | tee -a "$SUMMARY"
    rm -f "$TMP_CONFIG"
